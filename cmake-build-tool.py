...
def parse_arguments():
    ...
    parser.add_argument('-cpp', '--cpp_std', default='20', help='C++ language standard (e.g., c++17, c++20)')
    parser.add_argument('--compiler', default='gcc', choices=['gcc', 'clang'], help='Compiler to use: gcc or clang')
    parser.add_argument('--strip', action='store_true', help='Strip symbols from the final binary (Android only)')
...
def build_project(project_path, build_dir_base, system, ndk_path, arch, strip, cpp_std, clean_intermediate, compiler, c_std, verbose):
    ...
    # 为每个架构创建独立的构建目录
    build_dir = os.path.join(build_dir_base, project_name, system, ARCH_CONFIG[system].get(arch, arch))
    ...
    if system == 'android':
        ...
        build_linux(cmake_source_path, temp_build_dir, arch, compiler, c_std, cpp_std, clean_intermediate, log_file, verbose)
    elif system == 'windows':
...
def build_linux(project_path, build_dir, arch, compiler, c_std, cpp_std, clean_intermediate, log_file, verbose):
    cmake_cmd = [
        "cmake",
        "-DCMAKE_SYSTEM_NAME=Linux",
        f"-DCMAKE_C_COMPILER={ARCH_CONFIG['linux'][arch][compiler]}",
        f"-DCMAKE_CXX_COMPILER={ARCH_CONFIG['linux'][arch][compiler]}",
...
def build_windows(project_path, build_dir, arch, compiler, c_std, cpp_std, clean_intermediate, log_file, verbose):
    ...
    cmake_cmd = [
        ...
        f"-DCMAKE_C_COMPILER={ARCH_CONFIG['windows'][arch][compiler]}",
        f"-DCMAKE_CXX_COMPILER={ARCH_CONFIG['windows'][arch][compiler]}",
        f"-DCMAKE_C_STANDARD={c_std}" if c_std else "",
        f"-DCMAKE_CXX_STANDARD={cpp_std}",
        static_link_flags,
        project_path
...
def merge_build_results(temp_dir, final_dir, clean=False):
    ...
    try:
        ...
        # 如果需要清理临时目录中的文件
        if clean:
            extensions = [".so", ".a", ".dll", ".lib", ".exe"]
            delete_all_except(temp_dir, extensions)
        ...
        if clean:
            extensions = [".so", ".a", ".dll", ".lib", ".exe"]
            delete_all_except(final_dir, extensions)
